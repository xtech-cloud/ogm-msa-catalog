...
.PHONY: clean
clean:
	rm -rf /tmp/ogm-catalog.db

.PHONY: call
call:
	gomu --registry=etcd --client=grpc call xtc.ogm.catalog Healthy.Echo '{"msg":"hello"}'

.PHONY: post
post:
	curl -X POST -d '{"msg":"hello"}' localhost/ogm/catalog/Healthy/Echo                                                                                     1
